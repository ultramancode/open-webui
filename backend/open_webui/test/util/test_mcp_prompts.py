...
def setup_process_chat_payload_mocks(monkeypatch):
    ...
    monkeypatch.setattr(middleware_module, 'has_connection_access', lambda user, connection: True)
    ...
    monkeypatch.setattr(middleware_module, 'get_sorted_filter_ids', lambda *args, **kwargs: [])
    monkeypatch.setattr(
        middleware_module,
        'Functions',
        SimpleNamespace(get_functions_by_ids=lambda ids: []),
    )
    monkeypatch.setattr(
        middleware_module,
    ...
    monkeypatch.setattr(middleware_module, 'Chats', SimpleNamespace(get_chat_folder_id=lambda *args: None))
    monkeypatch.setattr(
        middleware_module,
        'Folders',
        SimpleNamespace(get_folder_by_id_and_user_id=lambda *args: None),
    )
